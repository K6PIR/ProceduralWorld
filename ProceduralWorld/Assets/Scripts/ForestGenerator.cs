...
public class ForestGenerator : MonoBehaviour {
    public static void AddTrees(Forest mainForest, ForestSettings settings, MeshSettings meshSettings, Vector2 coord,
            ...
            for (int x = 0; x < width; x++) {
                for (int i = 0; i < settings.forestElements.Length; i++) {
                    if (heightMap.values[x, y] > settings.forestElements[i].minHeight &&
                        ...
                        CanPlantTree(mainForest, new Vector2(x, y), settings.forestElements[i].separation) &&
                        ...
                        newForestElement.prefab.transform.position = newPosition;
                        mainForest.trees.Add(new Vector2(x, y), newForestElement);
                    }
                }
            }
        }
